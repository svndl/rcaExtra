...
switch testSettings.domain
    ...
    case 'freq'
        [nBins, nRc, nCnd, nSubj] = size(dataSet1.subjAvgImag);
        
        h0 = zeros(nBins, nRc, nCnd);
        pVal = ones(nBins, nRc, nCnd);
        ...
        for r = 1:nRc
            for c = 1:nCnd
                ...
                % reshape conditions
                data1Slice.subjAvgReal = reshape(d1, [nBins 1 nSubj]);
                data1Slice.subjAvgImag = reshape(d2, [nBins 1 nSubj]);
                ...
                if (~isempty(dataSet2))
                    ...
                    data2Slice.subjAvgReal = reshape(d3, [nBins 1 nSubj2]);
                    data2Slice.subjAvgImag = reshape(d4, [nBins 1 nSubj2]);
                end
    ...
    case 'sweep'
        % LLV - WORK IN PROGRESS - still adapting to sweeps, not working right
        % now
        
        [nBins, nFreqs, nRc, nCnd, nSubj] = size(dataSet1.subjAvgImag); % TODO FIX FIX FIX FIX FIX FIX FIX
        
        h0 = zeros(nBins, nFreqs, nRc, nCnd);
        pVal = ones(nBins, nFreqs, nRc, nCnd);
        % LLV: NOT IMPLEMENTED YET! - WORK IN PROGRESS, returning NaNs
        statResults.sig = h0;
        statResults.pValues = pVal;
        warning([newline newline 'STATS NOT IMPLEMENTED YET FOR SWEEPS, RETURNING NaNs' newline newline]);
        return
        ...
        for r = 1:nRc
            for c = 1:nCnd
                ...
                d2 = dataSet1.subjAvgImag(:, :, r, c, :);
                % reshape conditions
                data1Slice.subjAvgReal = reshape(d1, [nBins nFreqs 1 nSubj]);
                data1Slice.subjAvgImag = reshape(d2, [nBins nFreqs 1 nSubj]);
                ...
                if (~isempty(dataSet2))
                    ...
                    data2Slice.subjAvgReal = reshape(d3, [nBins nFreqs 1 nSubj2]);
                    data2Slice.subjAvgImag = reshape(d4, [nBins nFreqs 1 nSubj2]);
                end
                if (isfield(testSettings, 'testWithin'))
                    [h0(:, r, c), pVal(:, r, c), ~] = rcaExtra_tSquaredWithin(data1Slice, data2Slice);
                else
