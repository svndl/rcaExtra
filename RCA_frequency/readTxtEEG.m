function [colHdr, freqsAnalyzed, dataMatrix] = readTxtEEG(datafile, chanToSave)
...
% dataMatrix      matrix containing the desired data
    ...
    for i = 1:size(dati{1,channelIdx})
        chan{1,i} = sscanf(dati{1, channelIdx}{i}, 'hc%d');
    end

    dati{1,channelIdx} = chan';
    ...
    % Restricts the running matrix to the selected electrodes
    if ~isempty(chanToSave)
        dataMatrix = dataMatrix(ismember(int16(dataMatrix(:, 2)), int16(chanToSave(:))),:); 
    end
    % Selects all bins but the 0th one (i.e. the average bin)    
    dataMatrix = dataMatrix(dataMatrix(:, 4)>0, :); 
    % Selects all trials but the 0th one (i.e. the average trial)    
    dataMatrix = dataMatrix(dataMatrix(:, 1)>0, :); 

    [freqsAnalyzed, tmpIdx] = unique(dati{1, harmIdx}(:));
    freqNum = nan(1, length(freqsAnalyzed));
    for f = 1:length(freqsAnalyzed)
        freqNum(f) = dati{1, freqIdx}(tmpIdx(f));
    end
    [tmp,tmpIdx] = sort(freqNum);
    freqsAnalyzed = freqsAnalyzed(tmpIdx);

    for m = 1:length(usCols)
        colHdr{m} = hdrFields{usCols(m), 1};
    end
     % Computes amplitudes in final column
    dataMatrix(:, end + 1) = sqrt(dataMatrix(:, end - 1).^2 + dataMatrix(:, end).^2);
