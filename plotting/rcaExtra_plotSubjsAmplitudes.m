function figureHandles = rcaExtra_plotSubjsAmplitudes(varargin)
% function plots subj amplitudes for each condition/rc for a number of
% harmonic multiples
        ...
        for rc = 1:nRCs           
            ...
            for np = 1:nPages                
                ...
                for ni = 1:nItems
                    ...
                    subjsAmp_real = cat(3, groups{ni}.dataToPlot.subjsRe{:, cndIdx});
                    subjsAmp_imag = cat(3, groups{ni}.dataToPlot.subjsIm{:, cndIdx});
                    ...
                    try
                        ...
                        %% add subjs 
                        [nF, ~] = size(subjAmps_to_plot);
                        for f = 1:nF
                            idx_over_1 = subjAmps_to_plot(f, :) > 0;
                            idx_over_11 = subjAmps_to_plot(f, :) > 1;
                            
                            subjAmps_to_plot_over_1 = subjAmps_to_plot(f, idx_over_1);
                            fprintf('Condition %d, %dF Percent over 1 %f \n', ni, f, 100*sum(idx_over_11)/sum(idx_over_1));

                            nSubj = size(subjAmps_to_plot_over_1, 2);
                            xCoords = repmat(xPos(f), [1, nSubj]) + linspace(-barWidth/2, barWidth/2, nSubj);
                            ax_scatter = scatter(xCoords, squeeze(subjAmps_to_plot_over_1)); hold on;
                            ax_scatter.Marker = 'x';
