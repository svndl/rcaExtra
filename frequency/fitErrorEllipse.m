function [ampDiff, phaseDiff, zSNR, errorEllipse_out] = fitErrorEllipse(xyData, ellipseType, makePlot, returnRad)
...
%   mrC in tools/fitting/fitErrorEllipse
    ...
    switch ellipseType
        ...
        case {'SEMellipse' 'SEM'} % default
            a = sqrt(larger_eigenval/N); % contour at stdDev/sqrt(N)
            b = sqrt(smaller_eigenval/N);
        case '95CI'
    ...
    errorEllipse_out = [ellipse_x_r;ellipse_y_r]' * R;

    %Shift to be centered on mean coordinate
    errorEllipse = bsxfun(@plus,errorEllipse_out,meanXy);
