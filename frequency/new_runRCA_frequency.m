function rcaResult = new_runRCA_frequency(rcaSettings_current, sensorData, cellNoiseData1, cellNoiseData2)
% Alexandra Yakovleva, Stanford University 2012-2020.
    ...
    if (~exist(savedFile, 'file'))
        [rcaData, W, A, Rxx, Ryy, Rxy, dGen, ~] = ...
            new_rcaBase_Run(dataSlice, rcaSettings_current.nReg, rcaSettings_current.nComp); 
        covData.Rxx = Rxx;
        covData.Ryy = Ryy;
        covData.Rxy = Rxy;
        covData.dGen = dGen;
        try
            noiseData.lowerSideBand = new_rcaBase_Project(cellNoiseData1(:, rcaSettings_current.useCnds), W); 
            noiseData.higherSideBand = new_rcaBase_Project(cellNoiseData2(:, rcaSettings_current.useCnds), W);
        catch err
        ...
        try
            ...
            movefile(savedFile ,matFileRCA_old, 'f');
            rcaResult = new_runRCA_frequency(rcaSettings_current, dataSlice, cellNoiseData1(:, rcaSettings_current.useCnds), ...
                cellNoiseData2(:, rcaSettings_current.useCnds));            
        end
