function main_Standard_EdNeuro_analysis_freq

    %% define experimentInfo
    
    experimentName = 'EdNeuro_K2';
    ...
    % need to copy relevant fields to separate structure and pass it around    
    
    loadSettings = rcaExtra_getDataLoadingSettings(analysisStruct);
    loadSettings.useBins = 1:10;
    loadSettings.useFrequencies = analysisStruct.info.frequenciesLabels;
    
    
    % data reading settings
    [subjs, sensorData, cellNoiseData1, cellNoiseData2, info] = getRawData(loadSettings);    
    %% RCA training settings
    
%     settings_f1_raw.useFrequencies = analysisStruct.frequencyLabels{1};
%     settings_f1_raw.subjTag = analysisStruct.subjTag;
%     settings_f1_raw.useBins = analysisStruct.bins;
%     
%     settings_f1_rca = settings_f1_raw;
%     settings_f1_rca.useBins = 0;
%     settings_f1_rca.useFrequencies = analysisStruct.frequencyLabels{1};
% 
%     freqs_to_use_odd_f1 = [1 3 5 7 9]; % freq multiples to use for training
%     
%     settings_f1_odd.useFrequencies = analysisStruct.frequencyLabels{1}(freqs_to_use_odd_f1);
%     settings_f1_odd.subjTag = analysisStruct.subjTag;
%     settings_f1_odd.useBins = 0;
...
%     [subjList, sensorData_f1_rc, cellNoiseData1_f1, cellNoiseData2_f1, info_f1] = readselectiveDFTData(analysisStruct, settings_f1_rca);
%     [~, sensorData_f1_rc_odd, cellNoiseData1_f1_odd, cellNoiseData2_f1_odd, info_f1_odd] = readselectiveDFTData(analysisStruct, settings_f1_odd);
...
%     [~, sensorData_f1_raw, ~, ~, info_f1_raw] = readselectiveDFTData(analysisStruct, settings_f1_raw);    
...
%     freqs_to_use_c1_f1 = [1 3 5 7]; % freq multiples to use for training
%     
...
%     nConditions = numel(analysisStruct.conditionLabels);
...
%     close all;
    ...
    for nc = 1:nConditions
        ...
        % specify save file
        rcaSettings_cnd_f1.savedFile = [analysisStruct.destDataDir_RCA filesep 'rcaResults_Frequency_c' num2str(nc) '_f1.mat'];
        ...
        [projected_Conditions_rc{nc}, projected_Conditions_Subj_rc{nc}] = ...
            averageFrequencyData(cRawData', numel(analysisStruct.bins), numel(freqs_to_use_c1_f1), W_rc_ch{nc});
        
        % add topographies for plotting 
        projected_Conditions_rc{nc}.A = rcaStrct{nc}.A;
        
        % add condition label
        projected_Conditions_rc{nc}.label = analysisStruct.conditionLabels{nc};
        
        % test significance against zero
        stats_info = getBetweenConditionsStats(cRawData', numel(analysisStruct.bins), numel(freqs_to_use_c1_f1), W_rc_ch{nc}); 
        ...
        gcf_cnd_rcs = plotConditionSummaryGroup_freq(analysisStruct.frequencies(1), projected_Conditions_rc{nc});
        
        % name figures
        saveFigsStr = strcat(domain, '_rca_summary_Condition_', analysisStruct.conditionLabels{nc}, '_Comp_');
        saveFigStr_rcs = arrayfun(@(x) strcat(saveFigsStr, num2str(x), '.fig'), 1:nTotalComp, 'uni', false);
        % save figures
        cellfun(@(x, y) saveas(x, fullfile(analysisStruct.destDataDir_FIG, y)), gcf_cnd_rcs, saveFigStr_rcs', 'uni', false); 
    end
    ...
    for nc = 2:nConditions
        ...
        stats_info_c1 = getBetweenConditionsStats(sensorData_f1_raw(:, [cmpCnd nc])', numel(analysisStruct.bins), numel(freqs_to_use_c1_f1), W_rc_ch{nc});
        nTotalComp = size(W_rc_ch{nc}, 2);
        
        %% plot summary for each condition: RC topo, amplitude, frequency, significance
        gcf_cnd_rcs = plotConditionComparitionTwoGroups_freq(analysisStruct.frequencies(1), ...
            {projected_Conditions_rc{cmpCnd}, projected_Conditions_rc{nc}}, stats_info_c1);
        saveFigsStr = strcat(domain, '_rca_ComparingConditions_', analysisStruct.conditionLabels{cmpCnd}, '_', analysisStruct.conditionLabels{nc}, '_Comp_');
        saveFigStr_rcs = arrayfun(@(x) strcat(saveFigsStr, num2str(x), '.fig'), 1:nTotalComp, 'uni', false);
        cellfun(@(x, y) saveas(x, fullfile(analysisStruct.destDataDir_FIG, y)), gcf_cnd_rcs, saveFigStr_rcs', 'uni', false);         
    end
    ...
    nTotalComp = rcaSettings_f1.nComp + numel(rcaSettings_f1.chanToCompare);
    % 
    [bars_all, lolli_all] =  arrayfun(@(x) plotGroups_freq(analysisStruct.frequencies(1), analysisStruct.conditionLabels, projected_Conditions_rc{:}, x), ...
    ...
    % saving figures
    cellfun(@(x, y) saveas(x, fullfile(analysisStruct.destDataDir_FIG, y)), bars_all, saveFigStr_bars_rcs, 'uni', false);
    cellfun(@(x, y) saveas(x, fullfile(analysisStruct.destDataDir_FIG, y)), lolli_all, saveFigStr_loli_rcs, 'uni', false);
end